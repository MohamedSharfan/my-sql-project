...
VALUES (
    ...
    (
        ...
        'Student',
        '200212345670',
        'tharushi.wikramanayake1798@tec.lk',
        '07545678123'
    ),
    (
        'TG/2022/1502',
...
INSERT INTO Technical_Officer (id, hash_pwd)
VALUES ('TO_001', 'a8f2b9c7d4e6f1a2b3c4d5e6f7a8b9c0'),
...
INSERT INTO Admin (id, hash_pwd)
VALUES ('Ad_001', 'd4c3b2a1f6e5d4c3b2a1f0e9d8c7b6a5');
INSERT INTO Dean (id, appoint_date, office_room)
VALUES ('De_001', '2021-06-15', 'Office Room A201');
INSERT INTO Student (
        reg_no,
        year,
        dob,
...
SELECT
    ...
    w.week_no,
    DATE_ADD(@semester_start, INTERVAL (w.week_no - 1) WEEK) AS session_date,
    CASE 
    ...
    END AS session_type,
    CASE
        WHEN RIGHT(sc.reg_no, 1) IN ('0','1','2','3') THEN
            CASE WHEN w.week_no <= 13 THEN 'Present' ELSE 'Absent' END
        WHEN RIGHT(sc.reg_no, 1) IN ('4','5') THEN
            CASE WHEN w.week_no <= 10 THEN 'Present' ELSE 'Absent' END
        WHEN RIGHT(sc.reg_no, 1) IN ('6','7') THEN
            CASE 
                WHEN w.week_no <= 11 THEN 'Present'
                WHEN w.week_no IN (12,13) THEN 'Absent'
                ELSE 'Absent'
            END
        WHEN RIGHT(sc.reg_no, 1) IN ('8','9') THEN
            CASE 
                WHEN w.week_no <= 9 THEN 'Present'
                WHEN w.week_no IN (10,11) THEN 'Absent'
                ELSE 'Absent'
            END
        ELSE 'Present'
    END AS status,
...
UPDATE attendance a
JOIN medical m ON a.reg_no = m.reg_no
SET 
    a.status = 'Medical',
    a.ref_no = m.ref_no
WHERE 
    ...
    (
        (RIGHT(a.reg_no, 1) IN ('6','7') AND a.week_no IN (12,13)) OR
        (RIGHT(a.reg_no, 1) IN ('8','9') AND a.week_no IN (10,11))
    );
