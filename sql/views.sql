...
SELECT
    ...
    SUM(
        CASE
            WHEN a.status = 'Present' THEN cu.session_hour / COUNT(a.week_no)
            WHEN a.status = 'Medical' AND m.status = 'Approved' THEN cu.session_hour / COUNT(a.week_no)
            ELSE 0
        END
    ) AS attended_hours,
